...
(async () => {
  ...
  // Create a new browser context with viewport and user agent settings
  ...
  try {
    ...
    await page.goto('https://business.comcast.com/account/bill', {waitUntil: 'networkidle'});
    await sleep(2000);

    // Accept cookies if the banner appears
    try {
      console.log('Checking for cookie banner...');
      await page.waitForSelector('#onetrust-accept-btn-handler', {state: 'visible', timeout: 5000});
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        await page.waitForSelector('#user', {state: 'visible', timeout: 5000});
        ...
        await page.waitForSelector('#passwd, #password, input[type="password"]', {state: 'visible', timeout: 10000});
        emailEntered = true;
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
      try {
        ...
        await page.waitForSelector('#passwd, #password, input[type="password"]', {state: 'visible', timeout: 5000});
        await page.type('#passwd, #password, input[type="password"]', process.env.COMCAST_PASSWORD, {delay: 50});
        await sleep(500);
        await page.keyboard.press('Enter');

        // Wait for navigation to bill page
    ...
    await page.waitForSelector(pdfButtonSelector, {state: 'visible', timeout: 5000});
    ...
    const bills = await page.evaluate(() => {
      ...
      items.forEach((item) => {
        ...
        // Extract detailed bill line items
        bill.lineItems = [];
